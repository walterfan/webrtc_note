...
* **AGC1**: 传统的模拟/数字增益控制
* **AGC2**: 更新的自适应数字增益控制器（DSP 实现），本身并非神经网络，
  但内部使用了一个基于 GRU 的 RNN VAD（``agc2/rnn_vad/``）来辅助判断语音活动，从而更好地调整增益
...
* WebRTC 官方 NS 模块（``modules/audio_processing/ns/``）始终基于频谱减法 / 维纳滤波等**传统信号处理**方法
* 2020 年前后官方将旧的 C 实现**重写**为 C++ 的 ``NoiseSuppressor`` 类（社区俗称 “NS2”），
  但**算法内核不变，仍是维纳滤波，并未引入神经网络**
* 真正基于深度学习的降噪（RNNoise、DTLN 等）属于**第三方库或厂商自研**，不在官方 ``ns/`` 模块内
