...
带宽总是有限的, 当我们通过丢包, 延迟的度量指标估算出一个带宽值, 我们就要控制我们的发送速率, 确保它不会大于带宽而超发,
让发送速率不超过 SFU 或接收端的接收速率, 让发送的数据包不会因为超发而丢包, 从而保证接收的视频质量和流畅度.
...
- QP 持续偏低 → 触发分辨率提升
- ``qualityLimitationReason`` 统计指标反映当前限制原因：``none``, ``cpu``, ``bandwidth``, ``other``
